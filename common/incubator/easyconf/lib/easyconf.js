...
function(_, get_set, path, callsite) {
	...
	function safeAbsoluteRelativeRequire(filePath, dir) {
		var data = safeRequire(filePath);
		if (data === null)
			data = safeRequire(path.join( dir, filePath ));
		return data;
	}
	...
	Easyconf.prototype._addFile = function(confPath, options) {
		...
		if(basename === 'environmentalist.json') {
			...
			var data = safeAbsoluteRelativeRequire(confPath, options.calldir);
	...
	Easyconf.prototype._addPatternFiles = function(confPath, options) {
		...
		var env_data = safeAbsoluteRelativeRequire(envConfPath, options.calldir);
		//if(env_data === null) console.warn('easyconf : couldn’t find the given file "' + envConfPath + '" either in absolute or in relative !');
		if(env_data !== null) this._addStore( env_data, options);

		var envLocalConfPath = confRadix + '.' + env + '.local' + extension;
		var env_local_data = safeAbsoluteRelativeRequire(envLocalConfPath, options.calldir);
		...
		if(env_local_data !== null) this._addStore( env_local_data, options);
	};

	Easyconf.prototype._addEnv = function(envSpecPath, options) {
		var required_missing = false; // so far
		var data = {};
		var environmentalistSpec = safeAbsoluteRelativeRequire(envSpecPath, options.calldir);

		environmentalistSpec.forEach(function(envVarSpec) {
			data[envVarSpec.name] = process.env[envVarSpec.name];

			if(_.isUndefined(data[envVarSpec.name])) {
				data[envVarSpec.name] = envVarSpec.default; // XXX TOREVIEW
			}

			if(envVarSpec.easyconf_alias) {
				data[envVarSpec.easyconf_alias] = data[envVarSpec.name]; // XXX TOREVIEW
			}
	...
	Easyconf.prototype._addObject = function(data, options) {
		if(data instanceof Easyconf) {
			// grab data
			this._addStore(_.cloneDeep(data.get()), options);
		}
		else {
			// direct, with a copy
			this._addStore(_.cloneDeep(data), options);
		}
	};

	Easyconf.prototype._addStore = function(data, options) {
